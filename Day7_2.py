...
def rec(ans,curr,digits,position):
    if((ans==curr*digits[position] or ans==curr+digits[position] or ans==int(str(curr)+str(digits[position]))) and position==len(digits)-1):
        return 1
    if(position==len(digits)-1):
        ...
        conc=rec(ans,int(str(curr)+str(digits[position])),digits,position+1)
        if(plus==1 or mul==1 or conc==1):
            return 1
